...
var direction : float = 0.001f;

private var clapped : boolean = false;

static var gameStage : int = 0;
...
private var hasBeenLow : boolean = true;
private var clapTimer : float = 0.7f;
private var graceTimer : float = 0.1f;
private var currentTimer : float = 0.0f;
...
function Update () {
	if (gameStage == 0) {
		...
		if (clapped && graceTimer >= 0.0f) {
				graceTimer -= Time.deltaTime;
		}
		if (loudness > 0.5f && hasBeenLow) {
			hasBeenLow = false;
			if (!clapped){
				clapped = true;
				graceTimer = 0.1f;
			...
			else if (clapped && graceTimer <= 0.0f){
				gameStage = 1;
			}
		}
		...
		if (renderer.material.color.a <= 0 || renderer.material.color.a >= 1) {
			direction *= -1;
		}
		renderer.material.color.a -= direction;
	}
	else if (gameStage == 1){
		if (transform.position.y >= 0f) {
			transform.position.y -= 0.041f;
		}
		else {
			gameStage = 2;
		}
		...
		//stage 4 teaches about the Kinect
		if (clapTimer <= 0.0f) {
			clapped = false;
		}
		if (clapped && graceTimer >= 0.0f) {
		...
		//stage 5 teaches Targets/Bonuses
			if (clapTimer <= 0.0f) {
			clapped = false;
		}
		if (clapped && graceTimer >= 0.0f) {
